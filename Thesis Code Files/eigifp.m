...
if ( nargin > 1 + ~isempty(B) + ksetbyuser )
    ...
    I=find(strcmpi('DISP',names));
    if (~isempty(I))
        outputYesStr=options.(names{I});
    ...
    I=find(strcmpi('NORMA',names));
    if (~isempty(I))
        normA=options.(names{I});
    end

    I=find(strcmpi('NORMB',names));
    if (~isempty(I))
        normB=options.(names{I});
    end

    I=find(strcmpi('INITIALVEC',names));
    if (isempty(I))
        I=find(strcmpi('V0',names));
    end
    if (~isempty(I))
        X=options.(names{I});
        if( isempty(n) )
    ...
    I=find(strcmpi('TOLERANCE',names));
    if (isempty(I))
        I=find(strcmpi('TOL',names));
    end
    if (~isempty(I))
    ...
    I=find(strcmpi('INNERITERATION',names));
    if (isempty(I))
        I=find(strcmpi('INNERIT',names));
    end
    if (~isempty(I))
        m=options.(names{I});
        if ( m < 0 || m >= n )
    ...
    I=find(strcmpi('ILUTHRESH',names));
    if (isempty(I))
        I=find(strcmpi('ILU',names));
    end 
    if (~isempty(I))
        eta=options.(names{I});
        if ( eta < 0 )
    ...
    I=find(strcmpi('PRECONDITIONER',names));
    if (isempty(I))
        I=find(strcmpi('PRECON',names));
    end
    if (~isempty(I))
        L=options.(names{I});
    ...
    I=find(strcmpi('MAXEIG',names));
    if (~isempty(I))
        findMax=options.(names{I});
        if ( ~isnumeric(findMax) || ~((findMax==0) || (findMax==1)))
            fprintf(1,'Warning: opt.MAXEIG should be either 0 or 1. Reset to 0.\n');
        end             
    end
    
    I=find(strcmpi('USEPRECON',names));
    if (~isempty(I))
        usePreconIn=options.(names{I});
    ...
    I=find(strcmpi('MAXITERATION',names));
    if (isempty(I))
        I=find(strcmpi('MAXIT',names));
    end
    if (~isempty(I))
...
if (isnumeric(A))       %if given a matrix, use (A,B) or (-A,B) if maxeig =1
    ...
    if (findMax==1)     % here we use minusA and negate the results when done
        ...
        saveAfunc=0;
        if exist('Afunc123','var')   % we don't want to overwrite the user's global, so we save it
            saveAfunc=1;
            tempFunc=Afunc;
            clear Afunc123;
        end
        global Afunc123; %#ok<TLEV>
